...
export class GameEngine {
    ...
    recalculateFinancials(player: PlayerState) {
        ...
        if (prof) {
            ...
            // totalExpenses += liabilities (loop)
            // totalExpenses += loanDebt * 0.1

            // This implies `prof.expenses` is NOT the sum of liabilities. It might be just the base "Expenses" number from card?
            // Let's check Profession Interface: `expenses: number; // Base expenses (taxes + other fixed costs)`
            // Ah, `expenses` in Profession usually sums everything. 
            // BUT `recalculateFinancials` adds liabilities AGAIN? 
            // Line 309: `let totalExpenses = prof.expenses;`
            // Line 314 loop adds `l.expense`.
            // If `prof.expenses` includes Mortgage, and we add Mortgage again from liabilities, that's DOUBLE COUNTING.
            // UNLESS `prof.expenses` is ONLY Taxes + Other.
            // Let's check `professions.ts` again.
            // Entrepreneur: expenses: 6200. 
            // Taxes(1300) + Other(1500) + Car(700) + CC(1000) + School(500) + Mortgage(1200) = 6200.
            // So `prof.expenses` INCLUDES initial liabilities.
            // If we iterate `player.liabilities` and add them, we DOUBLE COUNT.
            // UNLESS `player.liabilities` is empty initially? No, initPlayer adds them.

            // BUG RISK: Existing code might be `totalExpenses = prof.expenses + children`. 
            // And IGNORES `player.liabilities` loop?
            // Let's look at Lines 306-322 again (previous view).
            // It calculates `liabilitiesExpense` but does it ADD it?
            // I need to see lines 321+.
