...
export const TransferModal = ({ isOpen, onClose, asset, players, myId, onTransfer }: TransferModalProps) => {
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-[#1e293b] w-full max-w-md rounded-3xl border border-slate-700 shadow-2xl p-6 relative flex flex-col max-h-[85vh]">
                <button onClick={onClose} className="absolute top-4 right-4 text-slate-500 hover:text-white transition-colors z-10">✕</button>

                <h2 className="text-xl font-bold text-white mb-2 shrink-0">Передача Актива</h2>

                <div className="flex-1 overflow-y-auto custom-scrollbar min-h-0 pr-1">
                    <div className="p-4 bg-slate-900/50 rounded-xl border border-slate-800 mb-6 shrink-0">
                    ...
                    <div className="space-y-4 mb-6">
                        <div className="text-sm text-slate-400 uppercase tracking-wider font-bold sticky top-0 bg-[#1e293b] py-2 z-10">Кому передать?</div>
                        ...
                        {availablePlayers.length > 0 ? (
                            <div className="grid grid-cols-1 gap-2 pb-2">
                                {availablePlayers.map(p => (
                                    ...
                                    >
                                        <div className="text-xl bg-slate-950 w-10 h-10 flex items-center justify-center rounded-lg border border-slate-800 shadow-inner group-hover:scale-110 transition-transform shrink-0">
                                            {p.token}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className={`font-bold truncate ${selectedPlayerId === p.id ? 'text-white' : 'text-slate-200'}`}>{p.name}</div>
                                        </div>
                ...
                <div className="flex gap-3 pt-4 border-t border-slate-700/50 mt-2 shrink-0">
