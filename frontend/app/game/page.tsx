        ...
        <div className={`h-screen w-screen bg-slate-900 ${room.status === 'playing' ? 'overflow-hidden' : 'overflow-y-auto custom-scrollbar'}`}>
            ...
            >
                {(isConnected: boolean, voiceError?: string | null) => {
                    ...
                    return (
                        <div className="h-screen w-screen flex flex-col bg-[#0b0e14] text-white overflow-hidden selection:bg-blue-500/30">

                            {/* [FIXED] TOP BAR: Voice Controls */}
                            <div className="flex-none z-50 px-4 py-2 bg-slate-900/95 backdrop-blur-md border-b border-white/10 flex items-center justify-between shadow-lg">
                                ...
                                <div className="flex-1 flex justify-center">
                                    {voiceError ? (
                                        <div className="text-red-400 text-[10px] font-bold truncate max-w-[200px]">{voiceError}</div>
                                    ) : isConnected ? (
                                        <div className="scale-90 origin-center">
                                            <VoiceControls onSpeakingChanged={setIsSpeaking} players={room.players} myId={userIdString} isHost={isHost} />
                                        </div>
                                    ) : (
                                        <div className="text-slate-500 text-xs italic">Подключение...</div>
                                    )}
                                </div>
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 flex flex-col items-center">
                                <div className="w-full max-w-5xl relative z-10 pt-4 pb-20">
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-20">
                                        ...
                                        <div className="lg:col-span-5 space-y-4">
                                            ...
                                            {/* Voice Controls: MOVED TO TOP BAR */}
                                                            ...
                                                            >
                                                                {DREAMS.filter(d => !room?.availableDreams || room.availableDreams.length === 0 || room.availableDreams.includes(d.name)).map(d => (
                                                                    <option key={d.id} value={d.name}>
                                                                        {d.name} {d.squareIndex ? `(№${d.squareIndex}) ` : ''}- ${d.cost.toLocaleString()}
                                                                    </option>
                                                                ))}
                                                            </select>
                                                            {/* Price Display */}
