...
function GameBoardContent({ roomId, userId, username, isHost, isTutorial, state, setState }: BoardProps & { state: any, setState: (s: any) => void }) {
    ...
    const handleMlmInvite = (targetId: string, slotIndex: number) => {
        ...
        socket.emit('mlm_invite', { roomId, userId: localPlayer.id, targetPlayerId: targetId, slotIndex });
    };
    ...
    return (
        (!state || !currentTurnPlayer) ? (
            ...
            (
                // Fragment or Div wrapper if needed? 
                // The child is a div with h-[100dvh]. We can just return it.
