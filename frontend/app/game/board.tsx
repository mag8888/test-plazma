...
function GameBoardContent({ roomId, userId, username, isHost, isTutorial, state, setState }: BoardProps & { state: any, setState: (s: any) => void }) {
    ...
    useEffect(() => {
        ...
        setActiveSpeakers(prev => {
            ...
            if (prev.length !== speakers.length) return speakers;
            if (prev.some((id, i) => id !== speakers[i])) return speakers;
            return prev;
        });
    }, [participants]);
    ...
    return (
        (!state || !currentTurnPlayer) ? (
            ...
            // VoiceRoom Removed - using Context from page.tsx
            // Only render board if we have state
            // We reuse isVoiceConnected logic derived above
            (
                // Responsive Layout Wrapper
                <div className="h-[100dvh] w-full bg-[#0f172a] text-white font-sans flex flex-col lg:flex-row overflow-hidden relative">
                                ...
                                <div className="bg-[#1e293b] w-full max-w-sm p-6 rounded-3xl border border-slate-700 shadow-2xl relative" onClick={e => e.stopPropagation()}>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {selectedPlayerForMenu.id !== localPlayer?.id ? (
                                            <button
                                                onClick={() => {
                                                    setTransferTarget(selectedPlayerForMenu);
                                        ...
                                        <div>
                                            ...
                                            <div className="text-xl font-bold text-white">
                                                {localPlayer.professionName === 'Entrepreneur' || localPlayer.professionName === 'ENTREPRENEUR' ? 'Предприниматель' :
                                                    (localPlayer.professionName === 'Manager' || localPlayer.professionName === 'MANAGER' ? 'Менеджер' : localPlayer.professionName)}
                                            </div>
                                        </div>
                                    </div>
                    ...
                    {/* LEFT SIDEBAR (Redesigned) - Flex Column */}
                    <div id="tutorial-players" className="hidden lg:flex flex-col w-[350px] h-full bg-[#0f172a]/50 relative z-40 overflow-hidden shrink-0 pt-0 gap-4 min-h-0">
...
export default function GameBoard(props: BoardProps) {
    ...
    // Initial State Sync
    useEffect(() => {
        console.log('🚀 MAIN BOARD COMPONENT MOUNTED - VERSION: V20_VOICE_RESTORED (Dev DB + Mic) 🚀');
