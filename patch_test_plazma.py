...
# 1. Insert POST handler
post_handler = """// Add new review
...
if "router.post('/reviews/add'" not in content:
    content = content.replace("router.get('/reviews'", post_handler + "router.get('/reviews'")

# 2. Replace button
old_button = '<a href="/admin" class="btn">← Назад</a>'
new_button = """<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
...
# 3. Add alerts
old_alerts = "${req.query.error === 'review_not_found' ? '<div class=\"alert alert-error\">❌ Отзыв не найден</div>' : ''}"
new_alerts = "${req.query.success === 'review_added' ? '<div class=\"alert alert-success\">✅ Отзыв успешно добавлен!</div>' : ''}\n        ${req.query.error === 'add_review_failed' ? '<div class=\"alert alert-error\">❌ Ошибка при добавлении отзыва</div>' : ''}\n        " + old_alerts

if "review_added" not in content:
...
if "addReviewModal" not in content:
    # Use simple string replacement for the end of file part
    content = content.replace("</body>", modal_html + "\n      </body>")

with open(target_file, "w") as f:
    f.write(content)

print("Successfully patched test-plazma/dist/admin/web.js")
